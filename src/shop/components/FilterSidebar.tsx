...
export const FilterSidebar = () => {
    ...
    const handleSizeChanged = (size: string) => {
        const newCurrentSizes = (currentSizes.includes(size))
            ? currentSizes.filter((s) => s !== size)
            : [...currentSizes, size];
        (newCurrentSizes.length === 0)
            ? searchParams.delete('sizes')
            : searchParams.set('sizes', newCurrentSizes.join());
            ...
            <div className="space-y-4">
                ...
                <RadioGroup defaultValue="" className="space-y-3">
                    <div className="flex items-center space-x-2">
                        <RadioGroupItem value="any" id="priceAny" onClick={() => handlePriceChanged('any')} checked={currentPrice === 'any'} />
                        <Label htmlFor="priceAny" className="text-sm cursor-pointer">Cualquier precio</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                        <RadioGroupItem value="0-50" id="price1" onClick={() => handlePriceChanged('0-50')} checked={currentPrice === '0-50'} />
                        <Label htmlFor="price1" className="text-sm cursor-pointer">$0 - $50</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                        <RadioGroupItem value="50-100" id="price2" onClick={() => handlePriceChanged('50-100')} checked={currentPrice === '50-100'} />
                        <Label htmlFor="price2" className="text-sm cursor-pointer">$50 - $100</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                        <RadioGroupItem value="100-200" id="price3" onClick={() => handlePriceChanged('100-200')} checked={currentPrice === '100-200'} />
                        <Label htmlFor="price3" className="text-sm cursor-pointer">$100 - $200</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                        <RadioGroupItem value="200+" id="price4" onClick={() => handlePriceChanged('+200')} checked={currentPrice === '+200'} />
